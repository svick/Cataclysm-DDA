...
class player_window
{
    ...
    protected:
        ...
        void info_print_label_value( int y, const std::string &label, int value_x,
                                     const std::string &value_format, Args &&... value_args ) {
            mvwprintz( w_info, y, 1, c_magenta, label );
            mvwprintz( w_info, y, value_x, c_magenta, value_format.c_str(),
        ...
        virtual void print_line( unsigned line, int y, bool selected ) override {
            ...
            if( line == 0 ) {
                ...
                if( selected ) {
                    ...
                    info_print_label_value( 4, metric ? _( "Carry weight (kg):" ) : _( "Carry weight (lbs):" ), 21,
                                            "%5.1f", convert_weight( p.weight_capacity() ) );
                ...
                if( selected ) {
                    ...
                    info_print_label_value( 5, _( "Throwing penalty per target's dodge:" ), 39, "%+3d",
                                            p.throw_dispersion_per_dodge( false ) );
                }
            } else if( line == 2 ) {
...
class effects_window : public player_window
{
    private:
        ...
        virtual void print_line( unsigned line, int y, bool selected ) override {
            trim_and_print( w_this, y, 1, getmaxx( w_this ) - 1, selected ? h_light_gray : c_light_gray,
                            effects[line].name );
...
void player::disp_info()
{
    ...
    // if encumberance or speed would scroll, but we have free space, move info lower
    int free_y = maxy - ( std::max( { effect_win_size_y, trait_win_size_y, skill_win_size_y } ) +
                          infooffsetybottom );
    int max_values = std::max( encumberance.values(), speed.values() );
    if( free_y > 0 && max_values > 8 ) {
        infooffsetytop += std::min( free_y, max_values - 8 );
    ...
    catacurses::window w_tip     = catacurses::newwin( 1, FULL_SCREEN_WIDTH,  VIEW_OFFSET_Y,
                                   0 + VIEW_OFFSET_X );
    catacurses::window w_stats   = catacurses::newwin( infooffsetytop - 2, 26,  1 + VIEW_OFFSET_Y,
                                   0 + VIEW_OFFSET_X );
    catacurses::window w_traits  = catacurses::newwin( trait_win_size_y, 26,
                                   infooffsetybottom + VIEW_OFFSET_Y, 27 + VIEW_OFFSET_X );
    catacurses::window w_encumb  = catacurses::newwin( infooffsetytop - 2, 26,  1 + VIEW_OFFSET_Y,
                                   27 + VIEW_OFFSET_X );
    catacurses::window w_effects = catacurses::newwin( effect_win_size_y, 26,
                                   infooffsetybottom + VIEW_OFFSET_Y, 54 + VIEW_OFFSET_X );
    catacurses::window w_speed   = catacurses::newwin( infooffsetytop - 2, 26,  1 + VIEW_OFFSET_Y,
                                   54 + VIEW_OFFSET_X );
